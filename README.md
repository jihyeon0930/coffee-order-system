...
- 주문 단건 및 회원별 목록 조회
- 주문 완료 Spring 이벤트 발행
- 완료 주문 기반 인기 메뉴 상위 10개 집계 API
- Cache Aside 방식의 Redis 캐시, 5분 TTL, 주문 커밋 후 캐시 무효화
- Redis 장애 시 DB 집계 조회 대체
...
완료된 전체 주문을 기준으로 판매 수량이 많은 메뉴 상위 10개를 반환한다. 동률이면 주문 횟수 내림차순, 메뉴 ID 오름차순으로 정렬한다. 결과는 Redis에 5분간 저장하며 주문이 정상 커밋되면 기존 캐시를 삭제한다. Redis를 사용할 수 없으면 DB 집계 결과를 반환한다.

## 향후 구현 계획

1. 실제 운영 규모 데이터로 인덱스 적용 전후 실행 계획 및 성능 비교
2. Spring Event와 `@Async`를 이용한 주문 후속 처리 분리
3. Kafka Producer/Consumer 기반 주문 이벤트 연동
4. k6 성능 테스트 작성
